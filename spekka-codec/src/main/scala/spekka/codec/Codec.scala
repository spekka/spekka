...
object Encoder {
  def apply[T](encoder: T => Array[Byte]): Encoder[T] = new Encoder[T] {
...
object Decoder {
  def apply[T](decoder: Array[Byte] => Try[T]): Decoder[T] = new Decoder[T] {
...
object Codec {
  implicit def codecFromEncoderAndDecoder[T](implicit encoder: Encoder[T], decoder: Decoder[T]): Codec[T] =
